...
class BNInception(nn.Module):
    ...
    def forward_middle_features(self, input):
        data_dict = dict()
        data_dict[self._op_list[0][-1]] = input
        middle_feature = []
        ...
        for op in self._op_list:
            if op[1] != 'Concat' and op[1] != 'InnerProduct':
                ...
                try:
                    data_dict[op[2]] = torch.cat(tuple(data_dict[x] for x in op[-1]), 1)
                    middle_feature.append(data_dict[op[2]])
                except:
                    for x in op[-1]:
                        print(x,data_dict[x].size())
                    raise
        return middle_feature, data_dict[self._op_list[-1][2]]
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        super(BNInception, self).load_state_dict(md, strict)
